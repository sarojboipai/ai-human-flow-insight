...
export const AIAgentNode = memo(({ data }: AIAgentNodeProps) => {
  ...
  return (
    <div 
      className={`relative bg-orange-50 rounded-lg border-2 ${borderClass} shadow-sm px-4 py-3 min-w-[140px] cursor-pointer hover:shadow-md transition-shadow`}
    ...
    >
      ...
      <Handle type="target" position={Position.Top} id="top" className="!bg-orange-400 !w-2 !h-2" />
      <Handle type="source" position={Position.Bottom} id="bottom" className="!bg-orange-400 !w-2 !h-2" />
      
      <div className="flex flex-col items-center gap-2">
        <div className="p-2 rounded-lg bg-orange-100">
          <Icon className="h-5 w-5 text-orange-600" />
        </div>
        <span className="text-xs font-medium text-center leading-tight text-orange-900">{data.label}</span>
      </div>
